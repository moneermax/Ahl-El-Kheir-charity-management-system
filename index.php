...
body{font-family:'Cairo',sans-serif;color:#fff;overflow:hidden;background:#0753ad;position:relative}
...
.page{min-height:100vh;display:flex;align-items:center;justify-content:center;padding:25px 34px 22px;position:relative;z-index:1}
...
.visual{text-align:center;padding:0 8px;zoom:.82}
.brand-logo{width:128px;height:128px;object-fit:contain;display:block;margin:0 auto 2px;border-radius:50%;background:#fff;padding:9px;filter:drop-shadow(0 12px 22px rgba(0,0,0,.18))}
...
@media(max-width:1100px){.wrapper{grid-template-columns:minmax(480px,1fr) minmax(390px,.82fr);gap:35px}.visual{padding:0;zoom:.76}.description{margin-bottom:28px}.goals{gap:8px}.goal{min-height:155px}.brand-logo{width:108px;height:108px}}
@media(max-width:900px){body{overflow:auto}.page{padding:75px 22px 65px}.wrapper{grid-template-columns:1fr;max-width:620px;gap:38px}.visual{order:1;zoom:1}.card-wrap{order:2}.footer{position:relative;bottom:auto;margin-top:25px}.language{top:18px;inset-inline-end:18px}.goals{max-width:600px}.login-card{max-width:520px}}
