...
try {
    if (dbFetchOne("SHOW TABLES LIKE 'employees'", [])) {
        ...
        $stats['expiring_contracts'] = dbFetchOne("SELECT COUNT(*) c FROM contracts WHERE status='active' AND end_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 30 DAY)", [])['c'] ?? 0;
...
.hr-dashboard-actions{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:12px;margin-bottom:20px}.hr-action-card{position:relative;display:flex;align-items:center;min-width:0;min-height:72px;padding:11px 13px;gap:10px;border:1px solid #e5e9ee;border-radius:10px;background:#fff;color:#212529;text-decoration:none;box-shadow:0 2px 8px rgba(0,0,0,.045);transition:transform .18s ease,box-shadow .18s ease,border-color .18s ease}.hr-action-card:hover{color:#212529;transform:translateY(-2px);border-color:#cbd5e1;box-shadow:0 5px 14px rgba(0,0,0,.08)}.hr-action-icon{width:40px;height:40px;min-width:40px;border-radius:9px;display:inline-flex;align-items:center;justify-content:center;background:#f4f6f8;font-size:16px}.hr-action-content{min-width:0;display:flex;flex-direction:column;line-height:1.25}.hr-action-title{font-size:.86rem;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.hr-action-subtitle{margin-top:3px;color:#6c757d;font-size:.69rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.hr-action-badge{position:absolute;top:7px;left:7px;min-width:21px}
/* HR dashboard: keep the global header clean; quick actions are displayed as dashboard cards below. */
.qa-top-bar .qa-actions{display:none !important}
@media (max-width:1199.98px){.hr-dashboard-actions{grid-template-columns:repeat(3,minmax(0,1fr))}}@media (max-width:767.98px){.hr-dashboard-actions{grid-template-columns:1fr 1fr}.grid-2{grid-template-columns:1fr}}@media (max-width:480px){.hr-dashboard-actions{grid-template-columns:1fr}.hr-action-card{min-height:68px}}
</style>
