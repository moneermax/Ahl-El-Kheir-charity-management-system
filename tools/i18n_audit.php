...
$ar = is_file($root . '/lang/ar.php') ? require $root . '/lang/ar.php' : [];
$en = is_file($root . '/lang/en.php') ? require $root . '/lang/en.php' : [];
$ar = is_array($ar) ? $ar : [];
$en = is_array($en) ? $en : [];
$arKeys = array_keys($ar); $enKeys = array_keys($en);
$missingEn = array_values(array_diff($arKeys, $enKeys));
$missingAr = array_values(array_diff($enKeys, $arKeys));

$extensions = ['php', 'js', 'html'];
$excluded = [
    DIRECTORY_SEPARATOR . 'TCPDF' . DIRECTORY_SEPARATOR,
...
foreach ($files as $file) {
    ...
    foreach ($excluded as $part) if (str_contains($normalized, $part)) continue 2;
...
$unknownKeys = [];
foreach ($keyHits as $path => $keys) foreach ($keys as $key) {
    if (!array_key_exists($key, $ar) || !array_key_exists($key, $en)) $unknownKeys[$path][] = $key;
}
$hardCodedCount = array_sum(array_map('count', $arabicHits));
$keyCount = array_sum(array_map('count', $keyHits));
$unknownCount = array_sum(array_map('count', $unknownKeys));
$ok = !$missingEn && !$missingAr && !$unknownKeys;

echo "AHL EL KHEIR I18N AUDIT\n" . str_repeat('=', 28) . "\n";
printf("Arabic catalog keys : %d\nEnglish catalog keys: %d\n", count($arKeys), count($enKeys));
printf("Missing English     : %d\nMissing Arabic      : %d\n", count($missingEn), count($missingAr));
printf("Key usages scanned  : %d\nUnknown key usages  : %d\n", $keyCount, $unknownCount);
