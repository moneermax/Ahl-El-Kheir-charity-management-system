...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if (!$errors) {
        ...
        try {
            ...
            dbExecute("INSERT INTO family_children (family_id,child_name,national_id,birth_date,gender,education_level,has_medical_needs,medical_need_id,medical_notes,nationality,health_status,health_status_other,psychological_state,psychological_state_other,monthly_sponsorship_value,extra_allowance,form_serial,is_active) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1)",
            [$newId,$childName,trim($_POST['child_national_id']??'')?:null,trim($_POST['child_birth_date']??'')?:null,$gender,trim($_POST['child_education_level']??'')?:null,0,null,null,trim($_POST['child_nationality']??'')!==''?trim($_POST['child_nationality']):'سودانية',$health,$healthOther,$psych,$psychOther,trim($_POST['child_medical_notes']??'')?:null,trim($_POST['child_monthly_sponsorship_value']??'')!==''?(float)str_replace(',','',$_POST['child_monthly_sponsorship_value']):null,trim($_POST['child_extra_allowance']??'')!==''?(float)str_replace(',','',$_POST['child_extra_allowance']):null,$formSerial]);
...
<div class="col-md-4"><label class="form-label">الحالة الصحية</label><select name="health_status" class="form-select" onchange="document.getElementById('create_health_other').style.display = this.value === 'أخرى' ? '' : 'none';"><?php foreach($AK_ORPHAN_OPTS['health'] as $o): ?><option value="<?php echo e($o); ?>"><?php echo e($o); ?></option><?php endforeach; ?></select></div><div class="col-md-4"><label class="form-label">حالة صحية أخرى</label><input type="text" name="health_status_other" id="create_health_other" class="form-control" style="display:none"></div><div class="col-md-4"><label class="form-label">الحالة النفسية</label><select name="psychological_state" class="form-select" onchange="document.getElementById('create_psych_other').style.display = this.value === 'أخرى' ? '' : 'none';"><?php foreach($AK_ORPHAN_OPTS['psych'] as $o): ?><option value="<?php echo e($o); ?>"><?php echo e($o); ?></option><?php endforeach; ?></select></div><div class="col-md-4"><label class="form-label">حالة نفسية أخرى</label><input type="text" name="psychological_state_other" id="create_psych_other" class="form-control" style="display:none"></div><div class="col-12"><label class="form-label">ملاحظات طبية</label><textarea name="child_medical_notes" class="form-control" rows="2"></textarea></div>
