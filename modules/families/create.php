...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if (!$errors) {
        ...
        try {
            db()->beginTransaction();
            dbExecute("INSERT INTO families (mother_name,mother_phone,mother_alt_phone,mother_job,mother_workplace,mother_national_id,mother_birth_date,father_name,father_death_date,city,district,address,monthly_need_amount,status,notes,supervisor_id,nanny_id,family_code,legacy_mother_first_letter,created_by,assigned_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            [$name,trim($_POST['mother_phone']??'')?:null,trim($_POST['mother_alt_phone']??'')?:null,trim($_POST['mother_job']??'')?:null,trim($_POST['mother_workplace']??'')?:null,trim($_POST['mother_national_id']??'')?:null,trim($_POST['mother_birth_date']??'')?:null,trim($_POST['father_name']??'')?:null,trim($_POST['father_death_date']??'')?:null,trim($_POST['city']??'')?:null,trim($_POST['district']??'')?:null,trim($_POST['address']??'')?:null,trim($_POST['monthly_need_amount']??'')!==''?(float)str_replace(',','',$_POST['monthly_need_amount']):null,array_key_exists($_POST['status']??'',$statuses)?$_POST['status']:'pending',trim($_POST['notes']??'')?:null,$supId,$nannyId,$code,$rawLetter,$uid,$supId]);
...
<div class="col-md-4"><label class="form-label">المهنة</label><input type="text" name="mother_job" class="form-control"></div><div class="col-md-4"><label class="form-label">مكان العمل</label><input type="text" name="mother_workplace" class="form-control"></div><div class="col-md-4"><label class="form-label">تاريخ ميلاد الأم</label><input type="date" name="mother_birth_date" class="form-control"></div>
