<?php
// modules/families/document_file.php - Authenticated streamer for family documents (storage/ is web-protected)
...
$id = (int)($_GET['id'] ?? 0);
$doc = dbFetchOne("SELECT d.*, f.nanny_id, f.supervisor_id, f.legacy_mother_first_letter FROM family_documents d LEFT JOIN families f ON f.id = d.family_id WHERE d.id = ?", [$id]);
if (!$doc) { http_response_code(404); exit(t('families.file_not_found')); }
if (!ak_family_user_in_scope($doc, $role, (int)Session::getUserId())) { http_response_code(403); exit(t('families.file_forbidden')); }
