...
function fina_schedule_approval_notification(int $collectionId,int $reviewerId,float $amount,string $currency): void{
    register_shutdown_function(function() use($collectionId,$reviewerId,$amount,$currency): void{
        try{
            $row=dbFetchOne("SELECT created_by,status,reviewed_by FROM fina_collections WHERE id=? LIMIT 1",[$collectionId]);
            if(!$row || $row['status']!=='approved' || (int)$row['reviewed_by']!==$reviewerId)return;
            ak_transaction_review_notify_user((int)$row['created_by'],'تم اعتماد تحصيل فينا الخير','تم اعتماد تحصيل فينا الخير رقم #'.$collectionId.' بمبلغ '.number_format($amount,2).' '.$currency.' وترحيله كالتزام مستقل بنسبة 100% لصالح فينا الخير.',APP_URL.'modules/accounting/fina_payment_review.php');
        }catch(Throwable $e){}
    });
}
