...
$pending=dbFetchAll("SELECT t.*, s.full_name AS sponsor_name, u.full_name AS creator_name,
    fa.allocation_mode AS fina_allocation_mode, fa.ahl_share_amount AS fina_ahl_share,
    fa.fina_share_amount AS fina_share, fa.ahl_admin_fee_amount AS fina_admin_fee,
    fa.ahl_net_amount AS fina_ahl_net
    FROM transactions t
    LEFT JOIN sponsors s ON s.id=t.sponsor_id
    LEFT JOIN users u ON u.id=t.created_by
    LEFT JOIN fina_payment_allocations fa ON fa.transaction_id=t.id
    WHERE t.status='pending_fm_review'
    ORDER BY t.submitted_at ASC, t.id ASC");
$history=dbFetchAll("SELECT t.*, s.full_name AS sponsor_name, u.full_name AS creator_name, r.full_name AS reviewer_name,
    fa.allocation_mode AS fina_allocation_mode, fa.ahl_share_amount AS fina_ahl_share,
    fa.fina_share_amount AS fina_share, fa.ahl_admin_fee_amount AS fina_admin_fee,
    fa.ahl_net_amount AS fina_ahl_net
    FROM transactions t
    LEFT JOIN sponsors s ON s.id=t.sponsor_id
    LEFT JOIN users u ON u.id=t.created_by
    LEFT JOIN users r ON r.id=t.fm_reviewed_by
    LEFT JOIN fina_payment_allocations fa ON fa.transaction_id=t.id
    WHERE t.status IN ('returned','posted','cancelled') AND t.fm_reviewed_at IS NOT NULL
    ORDER BY t.fm_reviewed_at DESC LIMIT 50");
...
<div class="card mb-4 fade-in"><div class="card-header text-white" style="background:#1b4d8f"><i class="fas fa-hourglass-half me-2"></i>دفعات بانتظار الاعتماد (<?php echo count($pending); ?>)</div><div class="card-body p-0"><div class="table-responsive"><table class="table table-hover align-middle mb-0"><thead class="table-light"><tr><th>الكود</th><th>التاريخ</th><th>المنشئ</th><th>الكفيل</th><th>الغرض</th><th>الإجمالي</th><th>أهل الخير</th><th>فينا الخير</th><th>الرسوم الإدارية</th><th>الإيصال</th><th class="text-center">المراجعة</th></tr></thead><tbody>
<?php if(!$pending): ?><tr><td colspan="11" class="text-center text-muted py-4">لا توجد دفعات معلقة حالياً.</td></tr><?php else: foreach($pending as $t): $hasFina=((float)($t['fina_share']??0)>0); $ahlShare=$hasFina?(float)$t['fina_ahl_share']:(float)$t['amount']; ?>
<tr><td><code><?php echo e($t['transaction_code']); ?></code></td><td><?php echo e($t['submitted_at']??$t['transaction_date']); ?></td><td><?php echo e($t['creator_name']??'—'); ?></td><td><?php echo e($t['sponsor_name']??'—'); ?></td><td><?php echo e($purposeLabels[$t['purpose']??'']??($t['transaction_type']??'')); ?><br><small class="text-muted"><?php echo e($t['payment_period']??''); ?></small></td><td><strong><?php echo number_format((float)$t['amount'],2); ?></strong> <?php echo e($t['currency_code']); ?></td><td><?php echo number_format($ahlShare,2); ?> <?php echo e($t['currency_code']); ?></td><td><?php if($hasFina): ?><strong><?php echo number_format((float)$t['fina_share'],2); ?></strong> <?php echo e($t['currency_code']); ?><?php else: ?><span class="text-muted">—</span><?php endif; ?></td><td><?php echo $hasFina?number_format((float)($t['fina_admin_fee']??0),2):'—'; ?></td><td><?php if(!empty($t['receipt_path'])):?><a class="btn btn-sm btn-outline-primary" target="_blank" href="<?php echo APP_URL; ?>modules/transactions/receipt_file.php?id=<?php echo (int)$t['id']; ?>"><i class="fas fa-eye"></i></a><?php else:?><span class="text-muted">—</span><?php endif;?></td><td style="min-width:230px"><form method="post" class="mb-1"><?php echo csrf_field(); ?><input type="hidden" name="approve_transaction" value="<?php echo (int)$t['id']; ?>"><button class="btn btn-sm btn-success w-100" type="submit" onclick="return confirm('اعتماد الدفعة وإنشاء القيد المحاسبي؟')"><i class="fas fa-check me-1"></i>اعتماد وترحيل</button></form><form method="post" class="d-flex gap-1"><?php echo csrf_field(); ?><input class="form-control form-control-sm" name="return_reason_<?php echo (int)$t['id']; ?>" placeholder="سبب الإرجاع" required><button class="btn btn-sm btn-danger" name="return_transaction" value="<?php echo (int)$t['id']; ?>" type="submit" onclick="return confirm('إرجاع الدفعة للمنشئ؟')"><i class="fas fa-undo"></i></button></form></td></tr>
<?php endforeach; endif; ?></tbody></table></div></div></div>
<div class="card fade-in"><div class="card-header"><i class="fas fa-history me-2"></i>سجل المراجعات الأخيرة</div><div class="card-body p-0"><div class="table-responsive"><table class="table table-sm table-hover align-middle mb-0"><thead class="table-light"><tr><th>التاريخ</th><th>الكود</th><th>المنشئ</th><th>المراجع</th><th>الإجمالي</th><th>أهل الخير</th><th>فينا الخير</th><th>الرسوم</th><th>الحالة</th><th>السبب</th></tr></thead><tbody>
<?php if(!$history): ?><tr><td colspan="10" class="text-center text-muted py-4">لا يوجد سجل مراجعات.</td></tr><?php else: foreach($history as $t): $hasFina=((float)($t['fina_share']??0)>0); $ahlShare=$hasFina?(float)$t['fina_ahl_share']:(float)$t['amount']; ?><tr><td><?php echo e($t['fm_reviewed_at']); ?></td><td><code><?php echo e($t['transaction_code']); ?></code></td><td><?php echo e($t['creator_name']??'—'); ?></td><td><?php echo e($t['reviewer_name']??'—'); ?></td><td><?php echo number_format((float)$t['amount'],2); ?></td><td><?php echo number_format($ahlShare,2); ?></td><td><?php echo $hasFina?number_format((float)$t['fina_share'],2):'—'; ?></td><td><?php echo $hasFina?number_format((float)($t['fina_admin_fee']??0),2):'—'; ?></td><td><?php echo $t['status']==='posted'?'<span class="badge bg-success">مرحّلة</span>':($t['status']==='returned'?'<span class="badge bg-warning text-dark">مُعادة</span>':'<span class="badge bg-secondary">ملغاة</span>'); ?></td><td><?php echo e($t['fm_review_reason']??'—'); ?></td></tr><?php endforeach; endif; ?></tbody></table></div></div></div>
