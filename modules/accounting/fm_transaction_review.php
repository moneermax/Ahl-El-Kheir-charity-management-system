    ...
    if (isset($_POST['approve_transaction'])) {
        ...
        elseif ((int)$t['created_by']===$uid && $role!=='admin') flash('error','لا يجوز للمنشئ اعتماد معاملته بنفسه.');
            ...
            try {
                ...
                dbExecute("UPDATE transactions SET status='posted', fm_reviewed_by=?, fm_reviewed_at=NOW(), fm_review_reason=NULL WHERE id=? AND status='pending_fm_review'",[$uid,$tid]);
                if (db()->rowCount()!==1) throw new RuntimeException('تعذر اعتماد الدفعة.');
    ...
    if (isset($_POST['return_transaction'])) {
        ...
        if($reason==='') flash('error','يجب كتابة سبب الإرجاع.');
            ...
            elseif((int)$t['created_by']===$uid && $role!=='admin') flash('error','لا يجوز للمنشئ إرجاع معاملته بنفسه.');
                ...
                try {
                    db()->beginTransaction();
                    dbExecute("UPDATE transactions SET status='returned', fm_reviewed_by=?, fm_reviewed_at=NOW(), fm_review_reason=? WHERE id=? AND status='pending_fm_review'",[$uid,$reason,$tid]);
                    if(db()->rowCount()!==1) throw new RuntimeException('تعذر إرجاع الدفعة.');
