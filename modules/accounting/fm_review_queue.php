<?php
// modules/accounting/fm_review_queue.php - Financial Manager Review Queue (v8: Notification-safe workflow)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verify_csrf()) {
        ...
        if ($sp) {
            ...
            try {
                dbExecute("INSERT INTO audit_log (user_id, action, entity_type, entity_id, old_values, new_values, ip_address, user_agent)
                    ...
                    [$uid, $sp_id, json_encode(['status' => 'pending']), json_encode(['status' => 'approved', 'txn_id' => $txnId, 'type' => $txnType], JSON_UNESCAPED_UNICODE), $_SERVER['REMOTE_ADDR'] ?? '', $_SERVER['HTTP_USER_AGENT'] ?? '']);
            } catch (Throwable $e) {}

            // Required downstream operational notification. Use role codes and an event-specific reference.
            try {
                ...
                foreach ($gmUsers as $u) {
                    ak_transaction_review_notify_event(
                        (int)$u['id'],
                        'اعتماد تحصيل مشرف #' . $sp_id,
                        'تم اعتماد تحصيل (' . $purLabel . ') بمبلغ ' . number_format($amount, 0) . ' بواسطة المدير المالي.',
                        APP_URL . 'modules/accounting/fm_review_queue.php?payment_id=' . $sp_id,
                        $sp_id,
                        'sponsor_payment_approved'
                    );
                }
            } catch (Throwable $e) {}
    ...
    if (isset($_POST['return_payment'])) {
        ...
        if ($note === '') {
            ...
            $sp = dbFetchOne("SELECT * FROM sponsor_payments WHERE id = ? AND status = 'pending'", [$sp_id]);
            if (!$sp) {
                flash('error', 'الدفعة غير موجودة أو لم تعد معلقة.');
            } else {
                ...
                if ($affected === 1) {
                    ...
                    // Required workflow notification: the submitting supervisor must act.
                    try {
                        ak_transaction_review_notify_event(
                            (int)$sp['supervisor_id'],
                            'تم إرجاع تحصيل مشرف #' . $sp_id,
                            'تم إرجاع التحصيل للمراجعة والتعديل. سبب الإرجاع: ' . $note,
                            APP_URL . 'modules/transactions/create.php?sponsor_id=' . (int)($sp['sponsor_id'] ?? 0) . '&returned_payment=' . $sp_id,
                            $sp_id,
                            'sponsor_payment_returned'
                        );
                    } catch (Throwable $e) {}

                    flash('success', 'تم إرجاع الدفعة للمشرف.');
                } else {
                    flash('error', 'تعذر إرجاع الدفعة لأنها تغيرت بالفعل.');
                }
            }
        }
        header('Location: ' . APP_URL . 'modules/accounting/fm_review_queue.php'); exit;
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                <thead class="table-light">
                    <tr>
                        <th>التاريخ</th><th>المشرف</th><th>الكفيل / اليتيم</th><th>الغرض / الفترة</th><th>المبلغ</th><th>الإيصالات</th><th class="text-center" style="width: 30%;">إجراء المراجعة</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if (!$payments): ?>
                    ...
                    <?php else: foreach ($payments as $p): ?>
                        <tr>
                            ...
                            <td>
                                <?php if ($p['sponsor_name']): ?><small class="text-muted">كفيل:</small> <?php echo e($p['sponsor_name']); ?> <code><?php echo e($p['sponsor_code'] ?? ''); ?></code><br><?php endif; ?>
                                <?php if (!empty($p['child_name'])): ?><small class="text-muted">يتيم:</small> <?php echo e($p['child_name']); ?><?php endif; ?>
                            </td>
                            <td><small><?php echo e($purposeLabels[$p['payment_type']] ?? $p['payment_type']); ?></small><?php if ($p['payment_period']): ?><small class="text-muted d-block"><?php echo e($p['payment_period']); ?></small><?php endif; ?></td>
                            <td><strong><?php echo number_format((float)$p['amount'], 0); ?></strong></td>
                            <td>
                                <?php if ($p['receipt_file_path']): ?><a href="<?php echo APP_URL . 'modules/transactions/receipt_sp.php?id=' . (int)$p['id']; ?>" target="_blank" class="btn btn-sm btn-outline-primary" title="إيصال البند"><i class="fas fa-eye"></i></a><?php endif; ?>
                                <?php if (!empty($p['unified_receipt_path'])): ?><a href="<?php echo APP_URL . 'modules/transactions/receipt_sp.php?id=' . (int)$p['id'] . '&kind=unified'; ?>" target="_blank" class="btn btn-sm btn-outline-secondary" title="الإيصال الموحّد"><i class="fas fa-layer-group"></i></a><?php endif; ?>
                                <?php if (!$p['receipt_file_path'] && empty($p['unified_receipt_path'])): ?><span class="text-danger">لا يوجد</span><?php endif; ?>
                            </td>
                            <td>
                                <form method="post" class="d-flex gap-1 align-items-center mb-1"><?php echo csrf_field(); ?><input type="hidden" name="approve_payment" value="<?php echo $p['id']; ?>"><button type="submit" class="btn btn-sm btn-success w-100" onclick="return confirm('اعتماد الدفعة وترحيلها للخزينة والقيود؟')"><i class="fas fa-check"></i> اعتماد</button></form>
                                <form method="post" class="d-flex gap-1 align-items-center"><?php echo csrf_field(); ?><input type="text" name="return_note_<?php echo $p['id']; ?>" class="form-control form-control-sm" placeholder="سبب الإرجاع..." required><button type="submit" name="return_payment" value="<?php echo $p['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('إرجاع الدفعة للمشرف؟')"><i class="fas fa-undo"></i></button></form>
                            </td>
                        </tr>
    ...
    <div class="card-body p-0">
        <div class="table-responsive">
            <table class="table table-sm table-hover align-middle mb-0">
                <thead class="table-light"><tr><th>التاريخ</th><th>المشرف</th><th>الكفيل</th><th>الغرض</th><th>المبلغ</th><th>الحالة</th><th>المراجع</th><th>الإيصالات</th><th>ملاحظات</th></tr></thead>
                <tbody>
                    <?php if (!$history): ?>
                    ...
                    <?php else: foreach ($history as $h): ?>
                        <tr>
                            ...
                            <td><small><?php echo e($purposeLabels[$h['payment_type']] ?? $h['payment_type']); ?></small><?php if ($h['payment_period']): ?><small class="text-muted d-block"><?php echo e($h['payment_period']); ?></small><?php endif; ?></td>
                            <td><?php echo number_format((float)$h['amount'], 0); ?></td>
                            <td><?php echo $h['status'] === 'approved' ? '<span class="badge bg-success">معتمد</span>' : '<span class="badge bg-danger">مرتجع</span>'; ?></td>
                            <td><?php echo e($h['reviewer_name'] ?? '—'); ?></td>
                            <td class="text-nowrap">
                                <?php if ($h['receipt_file_path']): ?><a href="<?php echo APP_URL . 'modules/transactions/receipt_sp.php?id=' . (int)$h['id']; ?>" target="_blank" class="btn btn-sm btn-outline-primary" title="إيصال البند"><i class="fas fa-eye"></i></a><?php endif; ?>
                                <?php if (!empty($h['unified_receipt_path'])): ?><a href="<?php echo APP_URL . 'modules/transactions/receipt_sp.php?id=' . (int)$h['id'] . '&kind=unified'; ?>" target="_blank" class="btn btn-sm btn-outline-secondary" title="الإيصال الموحّد"><i class="fas fa-layer-group"></i></a><?php endif; ?>
                            </td>
