...
if($_SERVER['REQUEST_METHOD']==='POST'&&verify_csrf()){
    if(isset($_POST['approve_payment'])){
        ...
        try{
            ...
            ak_transaction_review_apply_fina_intake($sp_id,$txnId);
            $journalId=ak_post_transaction_journal_fina_aware($txnId);if($journalId<=0)throw new RuntimeException('فشل إنشاء القيد المحاسبي.');
...
$payments=dbFetchAll("SELECT sp.*,u.full_name supervisor_name,spn.full_name sponsor_name,spn.sponsor_code,fc.child_name,fpi.allocation_mode,fpi.fina_share_amount FROM sponsor_payments sp JOIN users u ON u.id=sp.supervisor_id LEFT JOIN sponsorships spon ON spon.id=sp.sponsorship_id LEFT JOIN sponsors spn ON spn.id=COALESCE(sp.sponsor_id,spon.sponsor_id) LEFT JOIN family_children fc ON fc.id=spon.child_id LEFT JOIN fina_payment_intakes fpi ON fpi.sponsor_payment_id=sp.id WHERE sp.status='pending' ORDER BY sp.created_at ASC");
