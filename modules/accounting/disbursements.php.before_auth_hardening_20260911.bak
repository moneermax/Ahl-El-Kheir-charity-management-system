...
function disb_audit(int $userId, string $action, int $entityId, array $new): void {
    try {
        dbExecute("INSERT INTO audit_log (user_id, action_type, entity_type, entity_id, new_values, ip_address, user_agent, created_at)
...
function closeDisbursementWithReturn(int $disbursementId, int $nannyId, string $reason, string $returnReceiptPath, int $userId): array {
    ...
    try {
        ...
        if ($originalExpense) {
            ...
            $expenseAccount = dbFetchOne("SELECT id, code, name FROM accounts WHERE code = ? LIMIT 1", [$fallbackCode]);
            if (!$expenseAccount) {
        ...
        $cashAccount = dbFetchOne("SELECT id, code, name FROM accounts WHERE code = '1100' LIMIT 1");
        if (!$cashAccount) {
        ...
        <table class="table table-hover align-middle">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <?php elseif ($canManage && in_array($it['status'], ['paid', 'returned'], true)): ?>
                    <td class="text-center">
