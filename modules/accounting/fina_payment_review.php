...
function fina_external_source_label($note){
    ...
    if(!is_array($data))return 'مصدر خارجي';
...
<style>
.fina-review-table .fina-label{display:block;color:#6c757d;font-size:.74rem;font-weight:700;margin-bottom:2px}.fina-review-table .fina-value{font-weight:600}.fina-review-table .fina-review-actions{min-width:220px}.fina-review-table .fina-finance-grid{display:grid;grid-template-columns:repeat(2,minmax(140px,1fr));gap:10px}.fina-review-table .fina-finance-item{padding:8px 10px;border:1px solid #e5e9ef;border-radius:8px;background:#fff}.fina-review-table .fina-fina{color:#1b4d8f}.fina-review-table .fina-total{color:#212529}@media(max-width:991.98px){.fina-review-table .fina-finance-grid{grid-template-columns:1fr}.fina-review-table .fina-review-actions{min-width:190px}}
</style>
...
<div class="card mb-4"><div class="card-header text-white" style="background:#1b4d8f">طلبات بانتظار الاعتماد (<?php echo count($pending);?>)</div><div class="card-body p-0"><div class="table-responsive"><table class="table table-hover align-middle mb-0 fina-review-table"><thead class="table-light"><tr><th>بيانات التحصيل</th><th>القيمة</th><th>الإيصال</th><th class="text-center">المراجعة</th></tr></thead><tbody><?php if(!$pending):?><tr><td colspan="4" class="text-center text-muted py-4">لا توجد دفعات فينا الخير معلقة.</td></tr><?php else:foreach($pending as $p):$source=fina_external_source_label($p['other_source_note']);$description=preg_replace('/^FINA_ONLY\|/','',(string)$p['notes']);?><tr><td><div class="row g-2"><div class="col-md-3"><span class="fina-label">المصدر</span><span class="fina-value"><?php echo e($source);?></span></div><div class="col-md-3"><span class="fina-label">المشرف</span><span class="fina-value"><?php echo e($p['supervisor_name']);?></span></div><div class="col-md-2"><span class="fina-label">التاريخ</span><span class="fina-value"><?php echo e($p['payment_date']??date('Y-m-d',strtotime($p['created_at'])));?></span></div><div class="col-md-4"><span class="fina-label">ملاحظات</span><span class="fina-value"><?php echo e($description?:'—');?></span></div></div></td><td><div class="fina-finance-grid"><div class="fina-finance-item fina-total"><span class="fina-label">الإجمالي</span><strong><?php echo number_format((float)$p['amount'],2).' '.e($p['currency_code']);?></strong></div><div class="fina-finance-item fina-fina"><span class="fina-label">التزام فينا</span><strong><?php echo number_format((float)$p['amount'],2).' '.e($p['currency_code']);?></strong></div></div></td><td class="text-center align-middle"><?php if(!empty($p['receipt_file_path'])):?><a class="btn btn-sm btn-outline-primary" target="_blank" href="<?php echo APP_URL;?>modules/transactions/receipt_sp.php?id=<?php echo (int)$p['id'];?>"><i class="fas fa-eye"></i><span class="d-none d-md-inline ms-1">عرض</span></a><?php else:?><span class="text-muted">لا يوجد</span><?php endif;?></td><td class="fina-review-actions align-middle"><form method="post" class="mb-2"><?php echo csrf_field();?><button name="approve_fina" value="<?php echo(int)$p['id'];?>" class="btn btn-sm btn-success w-100" onclick="return confirm('اعتماد تحصيل فينا الخير وترحيله كالتزام 100%؟')"><i class="fas fa-check me-1"></i>اعتماد وترحيل</button></form><form method="post" class="d-flex gap-1"><input name="return_note" class="form-control form-control-sm" required placeholder="سبب الإرجاع"><?php echo csrf_field();?><button name="return_fina" value="<?php echo(int)$p['id'];?>" class="btn btn-sm btn-danger"><i class="fas fa-undo"></i> إرجاع</button></form></td></tr><?php endforeach;endif;?></tbody></table></div></div></div>
<div class="card"><div class="card-header">سجل دفعات فينا الخير الأخيرة</div><div class="card-body p-0"><div class="table-responsive"><table class="table table-sm"><thead><tr><th>تاريخ التحصيل</th><th>المصدر</th><th>المبلغ</th><th>العملة</th><th>الحالة</th><th>المراجع</th></tr></thead><tbody><?php if(!$history):?><tr><td colspan="6" class="text-center text-muted py-3">لا توجد دفعات معالجة بعد.</td></tr><?php else:foreach($history as $h):?><tr><td><?php echo e($h['payment_date']??'');?></td><td><?php echo e(fina_external_source_label($h['other_source_note']));?></td><td><?php echo number_format((float)$h['amount'],2);?></td><td><?php echo e($h['currency_code']);?></td><td><?php echo $h['status']==='approved'?'<span class="badge bg-success">معتمد</span>':'<span class="badge bg-danger">مرتجع</span>';?></td><td><?php echo e($h['reviewer_name']??'—');?></td></tr><?php endforeach;endif;?></tbody></table></div></div></div>
