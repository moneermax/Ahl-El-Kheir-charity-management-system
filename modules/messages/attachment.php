...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && ($_GET['action'] ?? '') === 'list') {
    ...
    if ($messageId <= 0 || !messaging_user_can_read($uid, $messageId)) {
        ...
        echo json_encode(['ok' => false, 'message' => 'ليس لديك صلاحية لعرض مرفقات هذه الرسالة.'], JSON_UNESCAPED_UNICODE);
        exit;
    }
    $rows = dbFetchAll(
        "SELECT a.id,a.message_id,a.original_name,a.mime_type,a.size_bytes,a.created_at,u.full_name uploader_name
         FROM message_attachments a JOIN users u ON u.id=a.uploader_user_id
         WHERE a.message_id=? OR a.message_id IN (SELECT id FROM messages WHERE parent_id=?) ORDER BY a.id ASC",
        [$messageId, $messageId]
    );
    foreach ($rows as &$row) {
        $row['download_url'] = APP_URL . 'modules/messages/download_attachment.php?id=' . (int)$row['id'];
        $row['size_label'] = ((int)$row['size_bytes'] >= 1048576)
            ? number_format(((int)$row['size_bytes'] / 1048576), 1) . ' MB'
            : number_format(max(1, (int)$row['size_bytes'] / 1024), 1) . ' KB';
    }
    unset($row);
    echo json_encode(['ok' => true, 'attachments' => $rows], JSON_UNESCAPED_UNICODE);
    exit;
}
...
$allowed = [
    ...
    'txt'=>['text/plain','application/octet-stream'],'jpg'=>['image/jpeg'],'jpeg'=>['image/jpeg'],'png'=>['image/png'],'gif'=>['image/gif'],'webp'=>['image/webp'],
    'zip'=>['application/zip','application/x-zip-compressed','application/octet-stream']
];
$ext = strtolower(pathinfo($original, PATHINFO_EXTENSION));
if (!isset($allowed[$ext])) {
    http_response_code(422); echo json_encode(['ok'=>false,'message'=>'نوع الملف غير مسموح.'],JSON_UNESCAPED_UNICODE); exit;
}
$finfo = new finfo(FILEINFO_MIME_TYPE);
$mime = (string)$finfo->file($file['tmp_name']);
if (!in_array($mime, $allowed[$ext], true)) {
    http_response_code(422); echo json_encode(['ok'=>false,'message'=>'محتوى الملف لا يتطابق مع نوعه المعلن.'],JSON_UNESCAPED_UNICODE); exit;
}
...
if (!is_dir($storage) && !mkdir($storage,0750,true) && !is_dir($storage)) {
    http_response_code(500); echo json_encode(['ok'=>false,'message'=>'تعذر إنشاء مساحة تخزين المرفقات.'],JSON_UNESCAPED_UNICODE); exit;
}
$stored = bin2hex(random_bytes(20)).'.'.$ext;
$target = $storage.DIRECTORY_SEPARATOR.$stored;
if (!move_uploaded_file($file['tmp_name'],$target)) {
    http_response_code(500); echo json_encode(['ok'=>false,'message'=>'تعذر حفظ المرفق.'],JSON_UNESCAPED_UNICODE); exit;
}
try {
    ...
    @unlink($target); error_log('message attachment insert: '.$e->getMessage());
    http_response_code(500); echo json_encode(['ok'=>false,'message'=>'تعذر تسجيل المرفق.'],JSON_UNESCAPED_UNICODE);
}
