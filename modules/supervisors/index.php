...
$pageTitle = 'إدارة المشرفين'; $active = 'supervisors';
...
<div class="welcome-section fade-in"><h2>إدارة المشرفين</h2><p>إدارة المشرفين، توزيع الحروف، متابعة أعباء العمل، وإدارة دورة حياة المشرف.</p><div class="quick-actions mt-3"><a href="<?php echo APP_URL; ?>modules/supervisors/create.php" class="btn btn-primary btn-sm"><i class="fas fa-user-plus me-1"></i> إضافة مشرف</a><a href="<?php echo APP_URL; ?>modules/supervisors/assign-letters.php" class="btn btn-secondary btn-sm"><i class="fas fa-font me-1"></i> توزيع الحروف</a><a href="<?php echo APP_URL; ?>modules/supervisors/reassign-letters.php" class="btn btn-warning btn-sm"><i class="fas fa-right-left me-1"></i> إعادة تعيين الحروف المحررة</a><a href="<?php echo APP_URL; ?>modules/supervisors/index.php?show_archived=<?php echo $showArchived?'0':'1'; ?>" class="btn btn-outline-dark btn-sm"><i class="fas <?php echo $showArchived?'fa-user-check':'fa-box-archive'; ?> me-1"></i><?php echo $showArchived?'إخفاء المؤرشفين':'عرض المؤرشفين'; ?></a></div></div>
...
<div class="card fade-in"><div class="card-header py-3 d-flex justify-content-between align-items-center"><span><i class="fas fa-user-tie me-2"></i><?php echo $showArchived?'المشرفون المؤرشفون وسجل المشرفين':'المشرفون الحاليون'; ?></span><span class="badge bg-primary"><?php echo count($supervisors); ?> مشرف</span></div><div class="card-body p-2 p-md-3"><div class="table-responsive"><table class="table table-hover align-middle mb-0 supervisor-management-table"><thead><tr><th class="text-center" style="width:42px">#</th><th>المشرف</th><th class="letters-cell">الحروف</th><th class="text-center">الأعباء</th><th class="text-center">الحالة</th><th class="text-center supervisor-actions">الإجراءات</th></tr></thead><tbody>
...
<tr><td class="text-center text-muted"><?php echo $i+1; ?></td><td class="supervisor-name"><strong><?php echo e($sup['full_name']); ?></strong><small class="d-block text-muted mt-1">@<?php echo e($sup['username']); ?></small></td><td class="letters-cell"><?php if(!empty($sup['letters'])): foreach(explode('، ',$sup['letters']) as $L): ?><span class="badge bg-light text-dark border me-1 mb-1"><?php echo e($L); ?></span><?php endforeach; else: ?><span class="text-muted">—</span><?php endif; ?></td><td class="text-center workload-cell"><a class="workload-link" href="<?php echo APP_URL; ?>modules/supervisors/sponsors.php?supervisor=<?php echo (int)$sup['id']; ?>" title="الكفلاء المرتبطون"><i class="fas fa-hand-holding-heart text-muted"></i> <?php echo (int)$sup['sponsor_count']; ?></a><span class="text-muted mx-1">·</span><span title="الأسر"><i class="fas fa-house-user text-muted"></i> <?php echo (int)$sup['family_count']; ?></span></td><td class="text-center"><span class="badge <?php echo $statusClass; ?>"><?php echo e(supervisorLifecycleStatusLabel($status)); ?></span></td><td class="text-center supervisor-actions"><div class="d-inline-flex flex-wrap justify-content-center gap-1"><a class="btn btn-sm btn-primary" title="الكفلاء" aria-label="الكفلاء" href="<?php echo APP_URL; ?>modules/supervisors/sponsors.php?supervisor=<?php echo (int)$sup['id']; ?>"><i class="fas fa-hand-holding-heart"></i></a><?php if(!$isFinal): ?><a class="btn btn-sm btn-warning" title="تعديل" aria-label="تعديل" href="<?php echo APP_URL; ?>modules/supervisors/edit.php?id=<?php echo (int)$sup['id']; ?>"><i class="fas fa-pen"></i></a><a class="btn btn-sm btn-info" title="الحروف" aria-label="الحروف" href="<?php echo APP_URL; ?>modules/supervisors/assign-letters.php?supervisor=<?php echo (int)$sup['id']; ?>"><i class="fas fa-font"></i></a><?php if(in_array($status,['active','suspended'],true)): ?><form method="post" action="<?php echo APP_URL; ?>modules/users/supervisor_status.php" data-confirm="<?php echo $status==='active'?'هل تريد إيقاف هذا المشرف مؤقتاً؟':'هل تريد إعادة تفعيل هذا المشرف؟'; ?>"><?php echo csrf_field(); ?><input type="hidden" name="id" value="<?php echo (int)$sup['id']; ?>"><button type="submit" class="btn btn-sm <?php echo $status==='active'?'btn-danger':'btn-success'; ?>" title="<?php echo $status==='active'?'إيقاف مؤقت':'تفعيل'; ?>" aria-label="<?php echo $status==='active'?'إيقاف مؤقت':'تفعيل'; ?>"><i class="fas <?php echo $status==='active'?'fa-pause':'fa-play'; ?>"></i></button></form><?php endif; ?><a class="btn btn-sm btn-outline-danger" title="مغادرة نهائية" aria-label="مغادرة نهائية" href="<?php echo APP_URL; ?>modules/users/supervisor_departure.php?id=<?php echo (int)$sup['id']; ?>"><i class="fas fa-user-slash"></i></a><?php else: ?><a class="btn btn-sm btn-success" title="إعادة المشرف إلى العمل" aria-label="إعادة المشرف إلى العمل" href="<?php echo APP_URL; ?>modules/supervisors/return.php?id=<?php echo (int)$sup['id']; ?>"><i class="fas fa-user-rotate"></i></a><span class="badge bg-dark align-self-center">نهائي</span><?php endif; ?></div></td></tr>
