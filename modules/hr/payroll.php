...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'generate') {
            ...
            if (!$policy) {
                throw new RuntimeException('لا توجد سياسة رواتب سارية حتى نهاية هذه الفترة. أنشئ أو اعتمد سياسة سارية قبل إنشاء المسير.');
            }
            ...
            try {
                $existsStmt = $pdo->prepare(
                    "SELECT id, status, allowances, overtime, deductions, payroll_policy_version_id
                     FROM payroll WHERE employee_id = ? AND month = ? AND year = ? LIMIT 1"
                );
                $insertStmt = $pdo->prepare(
                    "INSERT INTO payroll
                        (employee_id, month, year, basic_salary, allowances, overtime, deductions, net_salary, status, payroll_policy_version_id)
                     VALUES (?, ?, ?, ?, 0, 0, 0, ?, 'draft', ?)"
                );
                ...
                $policyLinkStmt = $pdo->prepare(
                    "UPDATE payroll
                     SET payroll_policy_version_id = ?
                     WHERE id = ? AND status = 'draft' AND payroll_policy_version_id IS NULL"
                );
                ...
                foreach ($eligible as $candidate) {
                    ...
                    if ($existing) {
                        if ($existing['status'] === 'draft') {
                            ...
                            if ($existing['payroll_policy_version_id'] === null) {
                                $policyLinkStmt->execute([(int)$policy['id'], (int)$existing['id']]);
                            }
                            $refreshed++;
                        } else {
...
<div class="pr-wrap">
    ...
    <div class="pr-card"><div class="pr-head">
        ...
        <div class="pr-note mb-3"><i class="fas fa-info-circle me-1"></i> يتم اختيار آخر راتب تاريخي صالح لنهاية الشهر. عند إنشاء مسير جديد يتم ربطه بسياسة الرواتب السارية لنهاية الفترة؛ هذا الربط لا يعيد حساب السجلات القديمة. العملة النظامية: <strong><?php echo htmlspecialchars(APP_CURRENCY_NAME_AR . ' (' . APP_CURRENCY_CODE . ')'); ?></strong>.</div>
        <?php if ($activePolicy): ?><div class="alert alert-info py-2"><i class="fas fa-gavel me-1"></i> السياسة السارية لهذه الفترة: <strong>رقم <?php echo (int)$activePolicy['version_no']; ?></strong> — من <?php echo htmlspecialchars($activePolicy['effective_from']); ?></div><?php else: ?><div class="alert alert-warning py-2"><i class="fas fa-exclamation-triangle me-1"></i> لا توجد سياسة رواتب سارية حتى <?php echo htmlspecialchars($periodEnd); ?>. لن يمكن إنشاء مسير جديد لهذه الفترة.</div><?php endif; ?>
        <div class="table-responsive"><table class="pr-table"><thead><tr><th>الموظف</th><th>العقد</th><th>الراتب الأساسي</th><th>السياسة</th><th>البدلات</th><th>الإضافي</th><th>الخصومات</th><th>الصافي</th><th>الحالة</th><th>الإجراء</th></tr></thead><tbody>
        <?php if(empty($payrolls)): ?><tr><td colspan="10" class="text-center py-5 text-muted"><i class="fas fa-file-invoice-dollar fa-2x mb-2 d-block"></i>لا توجد سجلات لهذه الفترة. اضغط «إنشاء المسير».</td></tr>
        <?php else: foreach($payrolls as $p): ?>
            <tr>
                ...
                <td><?php if(!empty($p['payroll_policy_version_no'])): ?><span class="pr-badge b-approved">V<?php echo (int)$p['payroll_policy_version_no']; ?></span><span class="salary-source"><?php echo htmlspecialchars($p['payroll_policy_effective_from']); ?></span><?php else: ?><span class="text-muted">تاريخي</span><?php endif; ?></td>
