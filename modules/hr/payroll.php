...
function payrollEligibleEmployees(PDO $pdo, string $periodEnd): array
{
    return dbFetchAll(
        "SELECT e.id, e.employee_code, e.full_name, e.hire_date,
         ...
         FROM employees e
         JOIN hr_employee_salary_history s ON s.employee_id = e.id
         ...
         ORDER BY e.full_name ASC, s.effective_from DESC, s.id DESC",
...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'generate') {
            ...
            try {
                $existsStmt = $pdo->prepare("SELECT id FROM payroll WHERE employee_id = ? AND month = ? AND year = ? LIMIT 1");
                $insertStmt = $pdo->prepare("INSERT INTO payroll (employee_id, month, year, basic_salary, allowances, overtime, deductions, net_salary, status) VALUES (?, ?, ?, ?, 0, 0, 0, ?, 'draft')");
                ...
                $pdo->commit();
                $message = "تم إنشاء {$inserted} مسير راتب كمسودة للفترة {$year}-" . str_pad((string)$month, 2, '0', STR_PAD_LEFT) . ($skipped ? " — تم تجاوز {$skipped} سجل موجود أو غير مؤهل." : '.');
            } catch (Throwable $e) {
            ...
            $row = dbFetchOne("SELECT id, status FROM payroll WHERE id = ? LIMIT 1", [$id]);
            ...
            $basic = (float)dbFetchOne("SELECT basic_salary FROM payroll WHERE id = ?", [$id])['basic_salary'];
...
$payrolls = dbFetchAll(
    "SELECT p.*, e.employee_code, e.full_name AS emp_name,
     ...
     LEFT JOIN hr_employee_salary_history s
       ON s.employee_id = p.employee_id
      AND s.effective_from <= LAST_DAY(STR_TO_DATE(CONCAT(p.year, '-', LPAD(p.month, 2, '0'), '-01'), '%Y-%m-%d'))
      AND (s.effective_to IS NULL OR s.effective_to >= LAST_DAY(STR_TO_DATE(CONCAT(p.year, '-', LPAD(p.month, 2, '0'), '-01'), '%Y-%m-%d')))
     LEFT JOIN hr_employee_contracts c ON c.id = s.contract_id
...
<div class="pr-wrap">
    ...
    <div class="pr-card"><div class="pr-body">
    ...
    <div class="d-flex gap-2 flex-wrap mb-3">
        ...
        <div class="pr-stat"><small>إجمالي الصافي</small><strong><?php echo number_format($summary['net'],2); ?> <span style="font-size:.75rem">EGP</span></strong></div>
    </div>
    ...
    <div class="pr-card"><div class="pr-head">
        ...
        <div class="pr-note mb-3"><i class="fas fa-info-circle me-1"></i> عند إنشاء المسير، يتم اختيار آخر راتب تاريخي صالح لنهاية الشهر، ويُستبعد الموظف إذا كانت حالته التاريخية في نهاية الشهر ضمن حالات الغياب المؤقت أو إنهاء الخدمة. <strong>employees.basic_salary</strong> لا يُستخدم كمصدر للراتب.</div>
        ...
        <?php else: foreach($payrolls as $p): ?>
            <tr>
                ...
                <td><strong><?php echo number_format((float)$p['basic_salary'],2); ?></strong><span class="salary-source"><?php echo htmlspecialchars($p['salary_currency'] ?: 'EGP'); ?> · <?php echo htmlspecialchars($p['salary_effective_from'] ?: ''); ?></span></td>
                ...
                <form method="POST"><input type="hidden" name="action" value="update_amounts"><input type="hidden" name="payroll_id" value="<?php echo $p['id']; ?>"><input type="hidden" name="month" value="<?php echo $selectedMonth; ?>"><input type="hidden" name="year" value="<?php echo $selectedYear; ?>">
                    <td><input class="money-input" type="number" min="0" step="0.01" name="allowances" value="<?php echo htmlspecialchars((string)$p['allowances']); ?>"></td>
                    <td><input class="money-input" type="number" min="0" step="0.01" name="overtime" value="<?php echo htmlspecialchars((string)$p['overtime']); ?>"></td>
                    <td><input class="money-input" type="number" min="0" step="0.01" name="deductions" value="<?php echo htmlspecialchars((string)$p['deductions']); ?>"></td>
                    <td><strong><?php echo number_format((float)$p['net_salary'],2); ?></strong><button class="pr-btn pr-outline" type="submit" style="display:block;margin-top:5px">حفظ المكونات</button></td>
                </form>
                <?php else: ?>
                    <td>+<?php echo number_format((float)$p['allowances'],2); ?></td><td>+<?php echo number_format((float)$p['overtime'],2); ?></td><td>-<?php echo number_format((float)$p['deductions'],2); ?></td><td><strong><?php echo number_format((float)$p['net_salary'],2); ?></strong></td>
                <?php endif; ?>
