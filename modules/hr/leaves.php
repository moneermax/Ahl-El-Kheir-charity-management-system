...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'request') {
            $emp_id = (int)$_POST['employee_id']; $type = $_POST['leave_type']; $start = $_POST['start_date']; $end = $_POST['end_date']; $reason = trim($_POST['reason']);
            $d1 = new DateTime($start); $d2 = new DateTime($end); $days = $d1->diff($d2)->days + 1;
            ...
            if ($leave) {
                ...
                $start = new DateTime($leave['start_date']); $end = new DateTime($leave['end_date']);
                $interval = new DateInterval('P1D'); $period = new DatePeriod($start, $interval, $end->modify('+1 day'));
                ...
                foreach ($period as $date) $stmt->execute([$leave['employee_id'], $date->format('Y-m-d')]);
        ...
        $message = t('hr.error', ['message' => $e->getMessage()]); $msg_type = 'error';
    }
}
...
if ($filter !== 'all') { $sql .= " WHERE l.status = ?"; $leaves = dbFetchAll($sql, [$filter]); } else { $sql .= " ORDER BY l.created_at DESC"; $leaves = dbFetchAll($sql, []); }
