...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($action, ['add', 'edit', 'suspend', 'activate'])) {
    try {
        if ($action === 'suspend' || $action === 'activate') {
            ...
            $stateCode = $action === 'suspend' ? 'suspended' : 'active';
            $state = hrGetEmploymentStateByCode($stateCode);
            if (!$state) {
                throw new RuntimeException('حالة التوظيف المطلوبة غير موجودة.');
            }
            hrSetEmploymentState(
                $emp_id,
                (int)$state['id'],
                Session::getUserID(),
                $action === 'suspend' ? 'Employee temporarily suspended' : 'Employee reactivated'
            );
            $message = $action === 'suspend' ? "تم إيقاف الموظف مؤقتاً" : "تم تفعيل الموظف";
            $action = 'list';
        } else {
            ...
            if ($action === 'add') {
                ...
                if ($create_account) {
                    ...
                    if (strlen($username) < 3) $username .= '00';
            ...
            if (isset($_FILES['contract_file']) && $_FILES['contract_file']['error'] === UPLOAD_ERR_OK) {
                ...
                if (in_array($ext, ['pdf', 'jpg', 'jpeg', 'png'], true)) {
                    ...
                    if (move_uploaded_file($file['tmp_name'], $upload_dir . $new_name)) {
                        $pdo->prepare("UPDATE employees SET contract_file_path = ? WHERE id = ?")->execute(['storage/hr_contracts/' . $new_name, $emp_id]);
                    }
                }
            }
            $action = 'list';
...
if ($action === 'delete' && $emp_id > 0) {
    ...
    if (in_array($currentUserRole, ['admin', 'sudo'], true)) {
        try {
            $state = hrGetEmploymentStateByCode('terminated');
            if (!$state) throw new RuntimeException('حالة إنهاء الخدمة غير موجودة.');
            hrSetEmploymentState($emp_id, (int)$state['id'], Session::getUserID(), 'Employee terminated');
...
<?php if ($action === 'list'): ?>
    ...
    <div class="fm-card">
        <div class="fm-card-head"><span>📋 سجل الموظفين</span><span class="badge-fm badge-green"><?php echo count($employees); ?> موظف</span></div>
        <div class="fm-card-body">
            <div class="table-responsive">
                <table class="fm-table">
                    <thead><tr><th>الكود</th><th>الاسم الكامل</th><th>القسم</th><th>المسمى الوظيفي</th><th>نمط العمل</th><th>الراتب</th><th>الحالة</th><th class="text-end">إجراءات</th></tr></thead>
                    <tbody>
                    ...
                    <?php else: foreach ($employees as $emp): ?>
                        <tr>
                            ...
                            <td class="text-end">
                                ...
                                <?php $currentUserRole = Session::getUserRole(); $isAdmin = in_array($currentUserRole, ['admin', 'sudo'], true); if ($emp['status'] !== 'terminated'): ?>
                                    <?php if ($emp['status'] === 'active'): ?>
                                        <form method="POST" style="display:inline;"><input type="hidden" name="action" value="suspend"><input type="hidden" name="employee_id" value="<?php echo $emp['id']; ?>"><button class="btn-fm btn-warning" style="font-size:0.75rem; padding:4px 8px;" title="إيقاف مؤقت" onclick="return confirm('هل أنت متأكد من إيقاف هذا الموظف مؤقتاً؟');"><i class="fas fa-pause"></i></button></form>
                                    <?php elseif ($emp['status'] === 'suspended'): ?>
                                        <form method="POST" style="display:inline;"><input type="hidden" name="action" value="activate"><input type="hidden" name="employee_id" value="<?php echo $emp['id']; ?>"><button class="btn-fm btn-success" style="font-size:0.75rem; padding:4px 8px;" title="تفعيل"><i class="fas fa-play"></i></button></form>
                                    <?php endif; if ($isAdmin): ?>
                                        <a href="?action=delete&id=<?php echo $emp['id']; ?>" class="btn-fm btn-danger-ghost" style="font-size:0.75rem; padding:4px 8px;" title="حذف نهائي" onclick="return confirm('تحذير: هذا الإجراء نهائي ولا يمكن التراجع عنه. هل أنت متأكد؟');"><i class="fas fa-trash"></i></a>
                                    <?php endif; ?>
                                <?php endif; ?>
                            </td>
                        </tr>
...
<?php else: ?>
    <div class="fm-card">
        <div class="fm-card-head"><span>📝 بيانات الموظف</span><a href="employees.php" class="btn-fm btn-ghost" style="background:#fff; color:#1b4d8f; font-size:0.8rem">العودة للقائمة</a></div>
        <div class="fm-card-body">
            <form method="POST" enctype="multipart/form-data">
                <div class="row g-3">
                    <div class="col-md-6"><label class="form-label">الاسم الكامل <span class="text-danger">*</span></label><input type="text" name="full_name" class="form-control" required value="<?php echo htmlspecialchars($employee['full_name'] ?? ''); ?>"></div>
                    <div class="col-md-3"><label class="form-label">كود الموظف <span class="text-danger">*</span></label><input type="text" name="employee_code" class="form-control" required value="<?php echo htmlspecialchars($employee['employee_code'] ?? ''); ?>"></div>
                    <div class="col-md-3"><label class="form-label">الرقم القومي</label><input type="text" name="national_id" class="form-control" value="<?php echo htmlspecialchars($employee['national_id'] ?? ''); ?>"></div>
                    <div class="col-md-3"><label class="form-label">تاريخ الميلاد</label><input type="date" name="birth_date" class="form-control" value="<?php echo htmlspecialchars($employee['birth_date'] ?? ''); ?>"></div>
                    <div class="col-md-3"><label class="form-label">النوع</label><select name="gender" class="form-select"><option value="">-- اختر --</option><option value="male" <?php echo ($employee['gender'] ?? '') === 'male' ? 'selected' : ''; ?>>ذكر</option><option value="female" <?php echo ($employee['gender'] ?? '') === 'female' ? 'selected' : ''; ?>>أنثى</option></select></div>
                    <div class="col-md-3"><label class="form-label">رقم الهاتف</label><input type="text" name="phone" class="form-control" value="<?php echo htmlspecialchars($employee['phone'] ?? ''); ?>"></div>
                    <div class="col-md-3"><label class="form-label">البريد الإلكتروني</label><input type="email" name="email" class="form-control" value="<?php echo htmlspecialchars($employee['email'] ?? ''); ?>"></div>
                    <div class="col-12"><label class="form-label">العنوان</label><input type="text" name="address" class="form-control" value="<?php echo htmlspecialchars($employee['address'] ?? ''); ?>"></div>
                    <div class="col-md-3"><label class="form-label">تاريخ التعيين <span class="text-danger">*</span></label><input type="date" name="hire_date" class="form-control" required value="<?php echo htmlspecialchars($employee['hire_date'] ?? ''); ?>"></div>
                    <div class="col-md-3"><label class="form-label">القسم</label><select name="department_id" class="form-select"><option value="">-- اختر القسم --</option><?php foreach ($departments as $dept): ?><option value="<?php echo $dept['id']; ?>" <?php echo ($employee['department_id'] ?? '') == $dept['id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($dept['name_ar']); ?></option><?php endforeach; ?></select></div>
                    <div class="col-md-3"><label class="form-label">المسمى الوظيفي <span class="text-danger">*</span></label><input type="text" name="position" class="form-control" required value="<?php echo htmlspecialchars($employee['position'] ?? ''); ?>"></div>
                    <div class="col-md-3"><label class="form-label">نوع التوظيف</label><select name="employment_type" class="form-select"><option value="full_time" <?php echo ($employee['employment_type'] ?? '') === 'full_time' ? 'selected' : ''; ?>>دوام كامل</option><option value="part_time" <?php echo ($employee['employment_type'] ?? '') === 'part_time' ? 'selected' : ''; ?>>دوام جزئي</option><option value="contract" <?php echo ($employee['employment_type'] ?? '') === 'contract' ? 'selected' : ''; ?>>عقد مؤقت</option><option value="volunteer" <?php echo ($employee['employment_type'] ?? '') === 'volunteer' ? 'selected' : ''; ?>>متطوع</option></select></div>
                    <div class="col-md-3"><label class="form-label">نمط العمل</label><select name="work_mode" class="form-select"><option value="remote" <?php echo ($employee['work_mode'] ?? '') === 'remote' ? 'selected' : ''; ?>>عن بُعد</option><option value="onsite" <?php echo ($employee['work_mode'] ?? '') === 'onsite' ? 'selected' : ''; ?>>في المقر</option><option value="hybrid" <?php echo ($employee['work_mode'] ?? '') === 'hybrid' ? 'selected' : ''; ?>>مختلط</option></select></div>
                    <div class="col-md-3"><label class="form-label">الراتب الأساسي</label><input type="number" step="0.01" name="basic_salary" class="form-control" value="<?php echo htmlspecialchars($employee['basic_salary'] ?? '0.00'); ?>"></div>
                    <div class="col-md-3"><label class="form-label">رقم الحساب البنكي</label><input type="text" name="bank_account" class="form-control" value="<?php echo htmlspecialchars($employee['bank_account'] ?? ''); ?>"></div>
                    <div class="col-md-3"><label class="form-label">الحالة</label><select name="status" class="form-select"><option value="active" <?php echo ($employee['status'] ?? '') === 'active' ? 'selected' : ''; ?>>نشط</option><option value="suspended" <?php echo ($employee['status'] ?? '') === 'suspended' ? 'selected' : ''; ?>>موقوف</option><option value="on_leave" <?php echo ($employee['status'] ?? '') === 'on_leave' ? 'selected' : ''; ?>>إجازة</option><option value="terminated" <?php echo ($employee['status'] ?? '') === 'terminated' ? 'selected' : ''; ?>>منهي الخدمة</option></select></div>
                    <div class="col-12"><label class="form-label">صورة العقد (PDF/صورة)</label><input type="file" name="contract_file" class="form-control" accept=".pdf,.jpg,.jpeg,.png"><small class="text-muted">ارفع نسخة من العقد الموقع</small><?php if ($employee && $employee['contract_file_path']): ?><div class="mt-2"><a href="<?php echo APP_URL . $employee['contract_file_path']; ?>" target="_blank" class="btn-fm btn-ghost btn-sm"><i class="fas fa-file-alt me-1"></i> عرض العقد الحالي</a></div><?php endif; ?></div>
                    <?php if ($action === 'add'): ?><div class="col-12"><div class="form-check"><input class="form-check-input" type="checkbox" name="create_account" id="create_account"><label class="form-check-label" for="create_account">إنشاء حساب نظامي للموظف</label></div></div><?php endif; ?>
                </div>
                <div class="mt-4 d-flex gap-2"><button type="submit" class="btn-fm btn-navy"><i class="fas fa-save me-1"></i> <?php echo $action === 'add' ? 'حفظ الموظف' : 'حفظ التعديلات'; ?></button><a href="employees.php" class="btn-fm btn-ghost">إلغاء</a></div>
                <input type="hidden" name="action" value="<?php echo htmlspecialchars($action); ?>">
                <?php if ($action === 'edit'): ?><input type="hidden" name="employee_id" value="<?php echo $emp_id; ?>"><?php endif; ?>
            </form>
        </div>
    </div>
<?php endif; ?>
