...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($action, ['add','edit','suspend','activate'], true)) {
    try {
        if ($action === 'suspend' || $action === 'activate') {
            ...
            if (!isset($statesByCode[$code])) throw new RuntimeException('حالة التوظيف المطلوبة غير موجودة.');
            hrSetEmploymentState($emp_id, (int)$statesByCode[$code]['id'], Session::getUserID(), $action === 'suspend' ? 'Employee temporarily suspended' : 'Employee reactivated');
            ...
            if (isset($_FILES['contract_file']) && $_FILES['contract_file']['error'] === UPLOAD_ERR_OK) {
                ...
                if (in_array($ext,['pdf','jpg','jpeg','png'],true)) { $new_name='contract_'.$emp_id.'_'.time().'.'.$ext; if (move_uploaded_file($file['tmp_name'],$upload_dir.$new_name)) $pdo->prepare('UPDATE employees SET contract_file_path=? WHERE id=?')->execute(['storage/hr_contracts/'.$new_name,$emp_id]); }
            }
            $action='list';
        }
    } catch (Throwable $e) { $message='خطأ: '.$e->getMessage(); $msg_type='error'; }
}

if ($action === 'delete' && $emp_id > 0) {
    if (in_array(Session::getUserRole(), ['admin','sudo'], true)) {
        try { if (!isset($statesByCode['terminated'])) throw new RuntimeException('حالة إنهاء الخدمة غير موجودة.'); hrSetEmploymentState($emp_id,(int)$statesByCode['terminated']['id'],Session::getUserID(),'Employee terminated'); $message="تم تغيير حالة الموظف إلى 'منهي الخدمة'"; }
        catch (Throwable $e) { $message='خطأ: '.$e->getMessage(); $msg_type='error'; }
    } else { $message='غير مصرح لك بهذا الإجراء'; $msg_type='error'; }
    $action='list';
}
...
?>
<style>
.fm-header{background:linear-gradient(135deg,#1b4d8f 0%,#2c5aa0 100%);color:#fff;padding:25px;border-radius:12px;margin-bottom:25px}.fm-header h1{margin:0;font-size:1.8rem}.fm-header p{margin:5px 0 0;opacity:.9}.fm-card{background:#fff;border-radius:12px;box-shadow:0 2px 10px rgba(0,0,0,.06);margin-bottom:20px;overflow:hidden}.fm-card-head{background:#1b4d8f;color:#fff;padding:12px 18px;font-weight:700;font-size:1rem;display:flex;justify-content:space-between;align-items:center}.fm-card-body{padding:20px}.fm-table{width:100%;border-collapse:collapse}.fm-table th,.fm-table td{padding:10px 12px;border-bottom:1px solid #eee;text-align:right;font-size:.9rem;vertical-align:middle}.fm-table th{background:#f8f9fa;font-weight:700;color:#1b4d8f}.fm-table tr:hover{background:#f8f9fa}.badge-fm{padding:4px 10px;border-radius:12px;font-size:.75rem;font-weight:700}.badge-green{background:#d4edda;color:#155724}.badge-amber{background:#fff3cd;color:#856404}.badge-red{background:#f8d7da;color:#721c24}.badge-gray{background:#e9ecef;color:#6c757d}.badge-blue{background:#d1ecf1;color:#0c5460}.badge-purple{background:#eadcff;color:#5b21b6}.btn-fm{display:inline-block;padding:6px 12px;border-radius:6px;border:none;cursor:pointer;font-weight:700;text-decoration:none;font-size:.8rem;margin:2px}.btn-navy{background:#1b4d8f;color:#fff}.btn-navy:hover{background:#143a6b;color:#fff}.btn-ghost{background:transparent;color:#1b4d8f;border:1px solid #1b4d8f}.btn-ghost:hover{background:#1b4d8f;color:#fff}.btn-warning{background:#ffc107;color:#000}.btn-warning:hover{background:#e0a800;color:#000}.btn-success{background:#28a745;color:#fff}.btn-success:hover{background:#218838;color:#fff}.btn-danger-ghost{background:transparent;color:#dc3545;border:1px solid #dc3545}.btn-danger-ghost:hover{background:#dc3545;color:#fff}.form-label{font-weight:600;color:#495057;margin-bottom:.5rem}.form-control,.form-select{border-radius:6px;border:1px solid #ced4da;padding:.6rem .9rem}.form-control:focus,.form-select:focus{border-color:#1b4d8f;box-shadow:0 0 0 .2rem rgba(27,77,143,.25)}.state-meta{display:block;font-size:.68rem;color:#6c757d;margin-top:2px}.state-pill{display:inline-flex;align-items:center;gap:6px;padding:5px 10px;border-radius:999px;background:#eef4ff;color:#1b4d8f;font-weight:700;font-size:.78rem}.state-pill.suspended{background:#fff0c2;color:#8a5a00}.state-pill.separation{background:#f8d7da;color:#721c24}.state-pill.probation{background:#eadcff;color:#5b21b6}.state-dot{width:7px;height:7px;border-radius:50%;background:currentColor}
</style>
