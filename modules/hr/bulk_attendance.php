...
try {
    ...
    /*
     * PHP is now the authoritative enforcement point for the former
     * attendance employment-state triggers. Validate every selected employee
     * before opening the write transaction so a rejected selection causes no
     * partial batch update.
     */
    $leaveIds = [];
    foreach ($ids as $id) {
        $eligibility = hrAttendanceEligibility($id, $selectedDate);
