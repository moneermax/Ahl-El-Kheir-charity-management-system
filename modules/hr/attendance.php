...
<?php foreach($rows as $row): $employee=$row['employee']; $att=$row['attendance']; $status=$row['status']; $onLeave=$row['on_leave']; $returned=$row['returned']; $bulkEligible=(!$onLeave && $status!=='on_leave'); $initials=mb_substr((string)$employee['full_name'],0,1,'UTF-8'); $searchText=mb_strtolower((string)$employee['full_name'].' '.(string)($employee['dept_name']??''),'UTF-8'); ?>
...
<td class="att5-check"><input type="checkbox" class="att5-employee" value="<?php echo (int)$employee['id']; ?>" <?php echo !$bulkEligible?'disabled title="الموظف غير مؤهل للحضور الجماعي"':''; ?>></td>
...
document.addEventListener('DOMContentLoaded',function(){
 ...
 const eligible=()=>Array.from(body.querySelectorAll('tr')).filter(r=>r.style.display!=='none').flatMap(r=>{const c=r.querySelector('.att5-employee');return c&&!c.disabled&&r.dataset.status!=='on_leave'?[c]:[]});
