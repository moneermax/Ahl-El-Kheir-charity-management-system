...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verify_csrf()) {
    ...
    if ($action === 'convert' && !empty($_POST['id'])) {
        $id = (int)$_POST['id']; $req = dbFetchOne("SELECT * FROM sponsor_requests WHERE id = ?", [$id]);
        if ($req && $req['status'] !== 'converted') {
            $n = (int)(dbFetchOne("SELECT COUNT(*) c FROM sponsors")['c'] ?? 0) + 1; $code = 'SP-' . str_pad((string)$n, 6, '0', STR_PAD_LEFT);
            dbExecute("INSERT INTO sponsors (full_name, phone, sponsor_code, status, sponsor_type, gender, preferred_payment_method, acquisition_source, brought_by_user_id, created_by) VALUES (?, ?, ?, 'active', 'individual', 'unknown', 'cash', ?, ?, ?)", [$req['sponsor_name'], $req['phone'], $code, $req['source'], $req['brought_by'], Session::getUserId()]);
