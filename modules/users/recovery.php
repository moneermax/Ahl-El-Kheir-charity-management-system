...
<form method="post" class="d-inline"><?php echo csrf_field(); ?><input type="hidden" name="request_id" value="<?php echo (int)$r['id']; ?>"><button name="approve" value="1" class="btn btn-sm btn-success" onclick="return confirm('الموافقة وتوليد كلمة مرور مؤقتة؟')"><i class="fas fa-check me-1"></i>موافقة</button> <button name="reject" value="1" class="btn btn-sm btn-danger" onclick="return confirm('رفض الطلب؟')"><i class="fas fa-ban me-1"></i>رفض</button></form>
<?php else: ?>—<?php endif; ?>
</td></tr>
