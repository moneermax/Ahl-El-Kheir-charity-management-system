<?php
// modules/transactions/index.php - Payments list + void + posted edit UI
...
$canEditPosted = in_array($role, ['admin', 'financial_manager', 'accountant'], true);
        ...
        if ($t && $t['status'] === 'posted') {
            ...
            if ($financialCommitted) {
                try {
                    ...
                    [Session::getUserId(), $tid,
                    json_encode(['code' => $t['transaction_code'], 'status' => 'posted'], JSON_UNESCAPED_UNICODE),
...
$totalSum = 0;
foreach ($filtered as $r) if ($r['status'] === 'posted') $totalSum += (float)$r['amount'];
...
<div class="col-md-2"><label class="form-label">الحالة</label><select name="tstatus" class="form-select"><option value="">الكل</option><option value="posted" <?php echo $fStatus === 'posted' ? 'selected' : ''; ?>>مرحّلة</option><option value="voided" <?php echo $fStatus === 'voided' ? 'selected' : ''; ?>>مبطلة</option></select></div>
<div class="col-md-2"><button class="btn btn-primary w-100"><i class="fas fa-search me-1"></i>بحث</button></div>
</form></div></div>
<div class="card fade-in"><div class="card-body"><div class="table-responsive">
...
<tbody>
<?php if (!$rows): ?><tr><td colspan="9" class="text-center text-muted py-4">لا توجد نتائج.</td></tr>
...
<td><?php echo e($r['receipt_number'] ?? '-'); ?><?php if (!empty($r['receipt_path'])): ?><br><a href="<?php echo APP_URL; ?>modules/transactions/receipt_file.php?id=<?php echo (int)$r['id']; ?>" target="_blank" class="badge bg-info text-decoration-none" title="عرض ملف الإيصال"><i class="fas fa-file-pdf me-1"></i>ملف</a><?php endif; ?></td>
<td><?php echo $r['status'] === 'posted' ? '<span class="badge bg-success">مرحّلة</span>' : '<span class="badge bg-danger">مبطلة</span>'; ?></td>
<td class="text-center" style="white-space:nowrap;">
<?php if (isset($jeMap[(int)$r['id']])): ?><a class="btn btn-sm btn-info" title="القيد المحاسبي" href="<?php echo APP_URL; ?>modules/accounting/journal.php?view=<?php echo $jeMap[(int)$r['id']]; ?>"><i class="fas fa-book"></i></a><?php endif; ?>
<?php if ($canEditPosted && $r['status'] === 'posted'): ?>
<button type="button" class="btn btn-sm btn-warning ak-edit-posted" title="تعديل الدفعة المرحّلة"
data-id="<?php echo (int)$r['id']; ?>" data-code="<?php echo e($r['transaction_code']); ?>" data-amount="<?php echo e((string)$r['amount']); ?>" data-month="<?php echo e((string)($r['payment_period'] ?? '')); ?>" data-purpose="<?php echo e((string)($r['purpose'] ?? '')); ?>" data-note="<?php echo e((string)($r['purpose_note'] ?? '')); ?>"><i class="fas fa-edit"></i></button>
<?php endif; ?>
...
<?php if ($pages > 1): ?><nav class="mt-2"><ul class="pagination pagination-sm justify-content-center"><?php foreach ($win as $i): ?><?php if ($i === 0): ?><li class="page-item disabled"><span class="page-link">…</span></li><?php else: ?><li class="page-item <?php echo $i === $page ? 'active' : ''; ?>"><a class="page-link" href="<?php echo e($qs(['page' => $i])); ?>"><?php echo $i; ?></a></li><?php endif; ?><?php endforeach; ?></ul></nav><?php endif; ?>
</div></div>

<!-- FM/accountant posted-transaction edit modal. The existing create.php handler remains the single financial edit implementation. -->
<div class="modal fade" id="akPostedEditModal" tabindex="-1" aria-hidden="true"><div class="modal-dialog"><div class="modal-content">
<form method="post" action="<?php echo APP_URL; ?>modules/transactions/create.php">
<div class="modal-header"><h5 class="modal-title">تعديل دفعة مرحّلة</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="إغلاق"></button></div>
<div class="modal-body">
<p class="mb-3">المعاملة: <strong id="akEditCode">—</strong></p>
<?php echo csrf_field(); ?>
<input type="hidden" name="edit_record" value="1"><input type="hidden" name="edit_src" value="posted"><input type="hidden" name="edit_id" id="akEditId">
<input type="hidden" name="edit_month" id="akEditMonth"><input type="hidden" name="edit_purpose" id="akEditPurpose"><input type="hidden" name="edit_note" id="akEditNote">
<div><label class="form-label">المبلغ</label><input type="number" step="0.01" min="0.01" class="form-control" name="edit_amount" id="akEditAmount" required></div>
<div class="alert alert-warning mt-3 mb-0">سيتم إبطال القيد المرحّل الحالي وإنشاء قيد بديل بالقيمة الجديدة بشكل ذري.</div>
</div>
<div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">إلغاء</button><button type="submit" class="btn btn-primary">حفظ التعديل</button></div>
</form></div></div></div>

<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
<script>
document.querySelectorAll('.ak-edit-posted').forEach(function(btn) {
    btn.addEventListener('click', function() {
        document.getElementById('akEditId').value = btn.dataset.id || '';
        document.getElementById('akEditCode').textContent = btn.dataset.code || '—';
        document.getElementById('akEditAmount').value = btn.dataset.amount || '';
        document.getElementById('akEditMonth').value = btn.dataset.month || '';
        document.getElementById('akEditPurpose').value = btn.dataset.purpose || '';
        document.getElementById('akEditNote').value = btn.dataset.note || '';
        var modal = new bootstrap.Modal(document.getElementById('akPostedEditModal'));
        modal.show();
    });
});
document.querySelectorAll('.ak-void-form').forEach(function(form) {
    form.addEventListener('submit', function(e) {
        ...
        if (typeof Swal !== 'undefined' && typeof Swal.fire === 'function') {
            Swal.fire({title:'تأكيد الإبطال',text:msg,icon:'warning',showCancelButton:true,confirmButtonColor:'#dc3545',cancelButtonColor:'#6c757d',confirmButtonText:'نعم، إبطال',cancelButtonText:'إلغاء',reverseButtons:true}).then(function(result){if(result.isConfirmed)proceed();});
