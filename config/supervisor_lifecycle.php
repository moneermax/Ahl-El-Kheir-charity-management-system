...
function startSupervisorLeave(
    ...
    try {
        ...
        if ($expectedReturnDate !== null && $expectedReturnDate !== '' && $expectedReturnDate < $startDate) throw new RuntimeException('Expected return date cannot precede the leave start date.');
        ...
        dbExecute("INSERT INTO supervisor_leaves
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", [$supervisorId, $leaveType, $startDate, $expectedReturnDate ?: null, $reason ?: null, $notes ?: null, $leaveStatus, $actorId, $actorId]);
        $leaveId = (int)db()->lastInsertId();
        if ($isToday) dbExecute("UPDATE users SET supervisor_status = 'on_leave', is_active = 0, updated_at = NOW() WHERE id = ?", [$supervisorId]);
        $pdo->commit();
        return $leaveId;
    } catch (Throwable $e) {
...
function activatePlannedSupervisorLeave(int $supervisorId, ?int $actorId): void
{
    ...
    $pdo->beginTransaction();
    try {
        $supervisor = dbFetchOne("SELECT u.id, COALESCE(u.supervisor_status, 'active') AS supervisor_status FROM users u JOIN roles r ON r.id = u.role_id WHERE u.id = ? AND r.code = 'supervisor' FOR UPDATE", [$supervisorId]);
        if (!$supervisor) throw new RuntimeException('Supervisor not found.');
        if ($supervisor['supervisor_status'] !== 'active') throw new RuntimeException('Supervisor is not active.');
        $leave = dbFetchOne("SELECT id, start_date FROM supervisor_leaves WHERE supervisor_id = ? AND status = 'planned' ORDER BY id DESC LIMIT 1 FOR UPDATE", [$supervisorId]);
...
function returnSupervisorFromLeave(int $supervisorId, ?int $actorId): void
{
    ...
    try {
        ...
        $leave = dbFetchOne("SELECT id FROM supervisor_leaves WHERE supervisor_id = ? AND status = 'active' ORDER BY id DESC LIMIT 1 FOR UPDATE", [$supervisorId]);
...
{
    $rows = dbFetchAll("SELECT sl.id, sl.supervisor_id, sl.letter_id, COALESCE(sl.gender, 'both') AS gender, sl.assigned_by FROM supervisor_letters sl WHERE sl.supervisor_id = ? FOR UPDATE", [$supervisorId]);
    foreach ($rows as $row) {
        $existing = dbFetchOne("SELECT id FROM supervisor_letter_assignment_history WHERE supervisor_letter_id = ? AND ended_at IS NULL LIMIT 1", [(int)$row['id']]);
        if (!$existing) {
            dbExecute("INSERT INTO supervisor_letter_assignment_history
                ...
                VALUES (?, ?, ?, ?, ?, 'permanent', 'legacy_backfill', NOW(), ?, ?)", [(int)$row['id'], (int)$row['supervisor_id'], (int)$row['letter_id'], $row['gender'], $row['assigned_by'] ?: null, $actorId, $reason]);
        } else {
            dbExecute("UPDATE supervisor_letter_assignment_history SET ended_at = NOW(), ended_by = ?, end_reason = ? WHERE id = ? AND ended_at IS NULL", [$actorId, $reason, (int)$existing['id']]);
        }
    }
...
function archiveSupervisor(int $supervisorId, ?int $actorId): int
{
    ...
    $pdo->beginTransaction();
    try {
        $supervisor = dbFetchOne("SELECT u.id, u.full_name, COALESCE(u.supervisor_status, CASE WHEN u.is_active = 1 THEN 'active' ELSE 'suspended' END) AS supervisor_status FROM users u JOIN roles r ON r.id = u.role_id WHERE u.id = ? AND r.code = 'supervisor' FOR UPDATE", [$supervisorId]);
        if (!$supervisor) throw new RuntimeException('Supervisor not found.');
        if (supervisorLifecycleIsFinal((string)$supervisor['supervisor_status'])) throw new RuntimeException('Supervisor is already departed or archived.');
        ...
        dbExecute("UPDATE users SET is_active = 0, supervisor_status = 'archived', password_hash = ?, updated_at = NOW() WHERE id = ?", [password_hash(bin2hex(random_bytes(32)), PASSWORD_DEFAULT), $supervisorId]);
        $pdo->commit();
        return $released;
    } catch (Throwable $e) {
        if ($pdo->inTransaction()) $pdo->rollBack();
