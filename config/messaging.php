...
function get_unified_unread_count(int $userId,string $role): int { return get_unread_message_count($userId,$role); }
function get_unread_count(int $userId,string $role): int{return get_unread_message_count($userId,$role);}
...
function send_system_notification(int $userId,string $title,string $message,string $type='info',?string $referenceType=null,?int $referenceId=null):int|false{try{$s=messaging_db()->prepare("INSERT INTO notifications(recipient_user_id,title,body,link,is_read,created_at) VALUES(?,?,?,?,0,NOW())");$link=null;if($referenceType&&$referenceId){if($referenceType==='message')$link=APP_URL.'modules/messages/index.php?message='.$referenceId;elseif($referenceType==='fina_collection')$link=APP_URL.'modules/accounting/fina_payment_history.php?id='.$referenceId;}$s->execute([$userId,$title,$message,$link]);return(int)messaging_db()->lastInsertId();}catch(Throwable $e){return false;}}
